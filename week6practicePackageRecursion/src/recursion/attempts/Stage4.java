...
public class Stage4 {
	...
	public static int binarySearch(int[] data, int target, int start, int end) {
		...
		if(data[median]==target)
			return median;
		if(data[median] > target)
			return binarySearch(data, target, start, median-1);
		return binarySearch(data, target, median+1, end); //to be completed
	}
	...
	public static int fastPower(int x, int n) {
		if (n==0)
			return 1;
		if(n%2!=0)
			return x* fastPower(x, (n-1)/2) * fastPower(x, (n-1)/2);
